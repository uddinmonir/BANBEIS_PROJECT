function [Er,Ar,Br,Cr,S] = IRKA_fl_index1(E1,J1,J2,J3,J4,B1,B2,C1,C2,Er, Ar, Br, Cr, S,r,maxiter,tol,w1,w2)
...
%Bt=S*(Er\Br);
%Ct=Cr*S;
%B = B*Bt' - expm(A*Time)*B*Bt'*expm((S)*Time);
%C = C'*Ct - expm(A'*Time)*C'*Ct*expm(S*Time);
S = - diag(S);
%disp('Finished Conventional IRKA')
A_omg= real((1i/pi)*logm((A+1i*w1*eye(size(A)))\(A+1i*w2*eye(size(A)))));
%Exp_A=expm(A*Time)*B;
...
for iter = 1:maxiter
    ...
    j = 1;
    % X = B*b;
    Ar_omg= real((1i/pi)*logm((Er\Ar+1i*w1*eye(size(Ar)))\(Er\Ar+1i*w2*eye(size(Ar)))));
    % Y = C'*c;
   ...
   pause(.1)
    ...
    B1til = W'*B1; 
    C1til = C1*V;
    Ar = J1til-J2til*(J4\J3til);
    Br = B1til-J2til*(J4\B2);
